...
fn get_board_coords (algebraic_notation: String) -> Vec<i32> {
    ...
    let col_number_index = col_names.chars().position(
        |y| 
            y == algebraic_notation
                .chars()
                .nth(0)
                .unwrap()
    ...
    let row_number = algebraic_notation
                .chars()
                .nth(1)
                .unwrap()
                as i32;
    return vec![row_number, 8 - col_number]
...
fn move_piece(mut board: Board, source: Vec<i32>, target: Vec<i32>) -> Board { //Function assumes valid board coordinates and valid move 
    /*
...
fn get_piece_movements (board: &Board, coords: &Vec<i32>, piece: &char) -> Vec<Vec<i32>> { 
    ...
    match piece.to_ascii_lowercase() {
        ...
        'r' => { // The rook moves in rows and cols [+-x], [+-y], until it hits a piece. 
...
fn active_player_is_in_check(board: Board) -> bool {
    todo!()
...
impl Game {
    ...
    pub fn new() -> Game {
        return Self::new_from_fen("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1".to_string())
    }
    ...
    pub fn make_move(&mut self, source: Vec<i32>, target: Vec<i32>) { //Assuming both square and target are Board coordinates <Vec<i32>> with length 2
        let available_moves = Self::get_available_moves(self.board.clone(), self.board.active_player);
        if available_moves.contains_key(&source) {
            if available_moves[&source].contains(&target) { // hopefully error free way of checking if the move is a valid move as dicteted by get_avialable_moves()
